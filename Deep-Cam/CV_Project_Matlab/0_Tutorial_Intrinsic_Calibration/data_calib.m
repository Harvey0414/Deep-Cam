...
l_gif = dir('*gif');
s_gif = size(l_gif,1);
l_png = dir('*png');  % 新增：检查PNG文件
s_png = size(l_png,1); % 新增：PNG文件数量

s_tot = s_ras + s_bmp + s_tif + s_pgm + s_jpg + s_ppm + s_gif + s_png;  % 更新总数计算
   ...
   calib_data.format_image = '0';
	...
	while calib_data.format_image == '0',
      ...
      switch first_char
          ...
          otherwise
              disp('Invalid image format');
              calib_data.format_image = '0'; % 重新询问格式
      end
   end;
