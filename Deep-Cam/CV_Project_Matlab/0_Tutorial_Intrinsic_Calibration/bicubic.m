...
for i = 1:num_images
    ...
    % 保存优化后的图像
%     output_name = sprintf('%s_bicubic%d.%s', basename, img_number, file_ext);
    output_name = sprintf('%s_bicubic%d.jpg', basename, img_number);
    imwrite(im_b, output_name); 
    fprintf('Saved enhanced image: %s\n', output_name);
end
